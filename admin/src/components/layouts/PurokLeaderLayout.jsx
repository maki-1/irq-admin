  ...
  return (
    <>
      <div className="flex flex-col items-center mb-6 pt-6 px-4">
        <img src={assets.DOLOGONLOGO} alt="Dologon" className="rounded-full object-cover mb-2" style={{ width: 80, height: 80 }} />
        <span style={{ fontFamily: "'Kaisei Decol', serif", color: '#156D07', fontSize: 18, fontWeight: 700, letterSpacing: 2 }}>
          DOLOGON
        </span>
        <span style={{ fontFamily: "'Hanken Grotesk', sans-serif", color: '#A18D8D', fontSize: 11, marginTop: 2 }}>
          Purok Leader
        </span>
      </div>
      ...
      <nav className="w-full flex flex-col gap-1 px-3 mt-6">
        {NAV.map(({ to, label, Icon }) => (
          <NavLink key={to} to={to} end={to === '/purok-leader'} onClick={onNavClick}
            className={({ isActive }) => `relative flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${isActive ? '' : 'hover:bg-gray-50'}`}
          >
            {({ isActive }) => (
              <>
                {isActive && <span className="absolute right-0 top-2 bottom-2 w-1 rounded-l-full" style={{ background: '#156D07' }} />}
                <Icon size={20} style={{ color: isActive ? '#156D07' : '#827575', flexShrink: 0 }} />
                <span style={{ fontFamily: "'Kaisei Decol', serif", fontSize: 15, fontWeight: 400, color: isActive ? '#156D07' : '#827575' }}>
                  {label}
                </span>
              </>
            )}
  ...
  return (
    <div className="flex items-center justify-between gap-3 mb-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 style={{ fontFamily: "'Kaisei Decol', serif", color: '#156D07', fontSize: 'clamp(18px, 2.5vw, 26px)', fontWeight: 400 }}>
            {title}
          </h1>
          {user?.purok && (
...
export default function PurokLeaderLayout({ title, children }) {
  ...
  return (
    <div className="flex min-h-screen" style={{ background: '#D9D9D9' }}>
      ...
      <div className={`fixed top-0 left-0 z-50 h-full flex flex-col lg:hidden transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
        style={{ width: 240, background: '#FFFFFF', boxShadow: '4px 0 16px rgba(0,0,0,0.15)' }}>
        <button className="absolute top-4 right-4 p-1 rounded-full hover:bg-gray-100" onClick={() => setSidebarOpen(false)}>
