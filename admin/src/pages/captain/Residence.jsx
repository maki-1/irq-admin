...
export default function CaptainResidence() {
  ...
  const handleExport = () => {
    ...
    a.href     = url;
    a.download = `residents_${filter === 'All' ? 'All' : filter.replace(/\s+/g, '_')}_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
          ...
          {FILTERS.map((f) => {
            ...
            return (
              <button
                ...
                style={{
                  fontFamily:  "'Hahmlet', sans-serif",
                  background:  isActive ? (cfg?.bg    || '#F0FDF4') : '#FFFFFF',
                  color:       isActive ? (cfg?.color || '#156D07') : '#827575',
          ...
          <div className="overflow-x-auto">
            <table className="w-full" style={{ borderCollapse: 'collapse', minWidth: 560 }}>
              ...
              <tbody>
                {loading && (
                  <tr>
                    <td colSpan={5} className="py-10 text-center text-sm" style={{ color: '#C0B0B0' }}>Loading…</td>
                  </tr>
                )}
                {!loading && visible.length === 0 && (
                  <tr>
                    <td colSpan={5} className="py-10 text-center text-sm" style={{ color: '#C0B0B0' }}>No profiles found</td>
                  </tr>
                )}
                {!loading && visible.map((p, idx) => {
                  ...
                  return (
                    <tr key={p._id} className="transition-colors hover:bg-gray-50"
                      ...
                      <td className="px-5 py-3" style={{ color: '#C0B0B0', fontSize: 13 }}>{globalIdx}</td>
